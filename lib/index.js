...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        verifiedPurchase = await paymentService.process({payment, order});
      } catch(e) {
        // If the Error requires canceling a Payment
        // mark the payment as VOIDED.
        if(e.details && e.details.void) {
          payment.status = PaymentStatus.VOIDED;
          await paymentService.db.save({payment});
          throw new BedrockError(
            'Payment voided.', Errors.Data, {public: true});
        }
        throw e;
      }
